      ...
      <div style={{ flex: 1, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 4 }}>
          <div style={{ fontSize: 16, fontWeight: 600, color: '#fff', lineHeight: 1 }}>
            <span style={{ fontSize: '.72em', opacity: .5, marginRight: 3 }}>{card.glyph}</span>{card.signo}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 4 }}>
          ...
          {card.imgs.length > 1 && <div style={{ fontSize: 7, padding: '1px 5px', borderRadius: 6, background: 'rgba(255,255,255,.07)', color: 'rgba(255,255,255,.3)' }}>{card.imgs.length} fotos</div>}
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', scrollbarWidth: 'none' }}>
      ...
      <div style={{ padding: '16px 18px 0', background: card.bg }}>
        ...
        <div style={{ fontStyle: 'italic', fontSize: 'clamp(12px,3.2vw,14px)', fontWeight: 300, color: 'rgba(255,255,255,.5)', marginTop: 2, marginBottom: 14 }}>{card.sub}</div>
      ...
      <div style={{ padding: '4px 18px 120px', background: card.bg }}>
...
export default function App() {
  ...
  const [from, setFrom] = useState('carousel')

  const isTodos = activeTab === 0 && screen !== 'detail'

  const goTab = useCallback(i => {
    setActiveTab(i)
    if (i === 0) setScreen('carousel')
    else { setListCat(CATS[i]); setScreen('list') }
  }, [])

  const openDetail = useCallback((c, fromScr) => {
    setCard(c); setFrom(fromScr)
    const ti = CATS.findIndex(k => k.l === c.cat)
  ...
  const goBack = useCallback(() => {
    setScreen(from)
    if (from === 'carousel') setActiveTab(0)
  }, [from])
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', zIndex: 1 }}>
      ...
      <div style={{ flexShrink: 0, zIndex: 10 }}>
        ...
        {/* Tabs */}
        <style>{`.ntab:hover{color:rgba(255,255,255,.6)!important;}`}</style>
        <div style={{ background: 'rgba(7,0,26,.97)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(255,255,255,.07)', display: 'flex', overflowX: 'auto', padding: '0 6px', scrollbarWidth: 'none' }}>
          {CATS.map((cat, i) => (
            <button key={i} className="ntab" onClick={() => goTab(i)} style={{
      ...
      <div style={{ flex: 1, overflow: 'hidden', minHeight: 0, display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 1 }}>
        ...
        {screen === 'carousel' && (
          <Carousel cards={CARDS} onSelect={c => openDetail(c, 'carousel')} />
        )}
        ...
        {screen === 'list' && listCat && (
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px 10px 20px', display: 'flex', flexDirection: 'column', gap: 8, scrollbarWidth: 'thin', scrollbarColor: 'rgba(255,255,255,.08) transparent' }}>
              {CARDS.filter(c => c.cat === listCat.l).map(c => (
                <ProductCard key={c.key} card={c} onClick={() => openDetail(c, 'list')} />
              ))}
            </div>
          </div>
        )}
        ...
        {screen === 'detail' && card && (
          <DetailScreen card={card} onBack={goBack} />
        )}
      </div>
