...
export default function Carousel({ cards, onSelect }) {
  ...
  const tx = useRef(0); const moved = useRef(false)
  ...
  useEffect(() => {
    const h = e => { if (e.key === 'ArrowLeft') step(-1); if (e.key === 'ArrowRight') step(1) }
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
      <style>{`
        ...
        .cc-glow { animation: pulse 2.5s ease-in-out infinite; border-color: transparent !important; }
        .cc-glow::before {
          content: ''; position: absolute; inset: -2px; z-index: -1; border-radius: 18px;
          background: conic-gradient(from var(--ang), transparent 0deg, var(--g1) 50deg, rgba(255,255,255,.9) 88deg, transparent 145deg, var(--g2) 205deg, rgba(255,255,255,.55) 248deg, transparent 325deg, var(--g1) 360deg);
        ...
        .dot-btn { width:5px;height:5px;border-radius:50%;background:rgba(255,255,255,.22);border:none;padding:0;cursor:pointer;transition:all .22s; flex-shrink:0; }
        .dot-btn.on { background:#f5d080;width:16px;border-radius:3px; }
        .arr-btn:hover { background: rgba(255,255,255,.18) !important; }
      `}</style>

      {/* Stage */}
      <div
        style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', perspective: '900px', overflow: 'visible', minHeight: 0 }}
        onTouchStart={e => { tx.current = e.touches[0].clientX; moved.current = false }}
        onTouchMove={e => { if (Math.abs(e.touches[0].clientX - tx.current) > 12) moved.current = true }}
        onTouchEnd={e => { const dx = e.changedTouches[0].clientX - tx.current; if (moved.current && Math.abs(dx) > 45) step(dx < 0 ? 1 : -1) }}
      >
        {POS.map(p => {
          const idx = ((cur + p.o) % n + n) % n
          const c = cards[idx]
          const isC = p.o === 0
          const g1 = hex2rgba(c.ac, 0.8), g2 = hex2rgba(c.ac, 0.22)

          return (
            <div
              key={p.o}
              className={isC ? 'cc-glow' : ''}
              onClick={() => { if (isC) onSelect(c); else if (p.o === -1) step(-1); else if (p.o === 1) step(1) }}
              style={{
                position: 'absolute',
                ...
                transition: 'transform .48s cubic-bezier(.4,0,.2,1), opacity .48s cubic-bezier(.4,0,.2,1), filter .48s cubic-bezier(.4,0,.2,1)',
                willChange: 'transform,opacity', touchAction: 'pan-y',
        ...
        {/* Arrows */}
        {[-1, 1].map(dir => (
          <button key={dir} className="arr-btn" onClick={() => step(dir)} style={{
      ...
      <div style={{ display: 'flex', gap: 6, justifyContent: 'center', padding: '8px 0 5px', flexShrink: 0 }}>
        {cards.map((_, i) => (
          <button key={i} className={`dot-btn${i === cur ? ' on' : ''}`} onClick={() => { if (!busy) setCur(i) }} />
        ))}
      </div>
    </div>
  )
